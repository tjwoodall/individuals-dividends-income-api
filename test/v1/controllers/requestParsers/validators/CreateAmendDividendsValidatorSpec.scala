...
class CreateAmendDividendsValidatorSpec extends UnitSpec with ValueFormatErrorMessages {
  ...
  class Test extends MockAppConfig {
    ...
    val validator                     = new CreateAmendDividendsValidator()
  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, validRawRequestBody)) shouldBe Nil
      }
    }

    "return NinoFormatError error" when {
      "an invalid nino is supplied" in new Test {
        validator.validate(CreateAmendDividendsRawData("A12344A", validTaxYear, validRawRequestBody)) shouldBe
    ...
    "return TaxYearFormatError error" when {
      "an invalid tax year is supplied" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, "20178", validRawRequestBody)) shouldBe
    ...
    "return RuleIncorrectOrEmptyBodyError error" when {
      "an empty JSON body is submitted" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, emptyRawRequestBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "a non-empty JSON body is submitted without any expected fields" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, nonsenseRawRequestBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "the submitted request body is not in the correct format" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, nonValidRawRequestBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError.copy(paths = Some(Seq("/stockDividend/grossAmount"))))
      }

      "return RuleTaxYearRangeInvalidError error" when {
        "an invalid tax year range is supplied" in new Test {
          validator.validate(CreateAmendDividendsRawData(validNino, "2019-21", validRawRequestBody)) shouldBe
      ...
      "return RuleTaxYearNotSupportedError error" when {
        "an invalid tax year is supplied" in new Test {
          validator.validate(CreateAmendDividendsRawData(validNino, "2018-19", validRawRequestBody)) shouldBe
      ...
      "mandatory fields are not provided" in new Test {
        ...
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, missingMandatoryFieldRequestBody)) shouldBe
    ...
    "return CountryCodeFormatError error" when {
      "an incorrectly formatted country code is submitted" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidCountryCodeRawRequestBody)) shouldBe
    ...
    "return CountryCodeRuleError error" when {
      "an invalid country code is submitted" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidCountryCodeRuleRawRequestBody)) shouldBe
    ...
    "return CustomerRefFormatError error" when {
      "an incorrectly formatted customer reference is submitted" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidCustomerRefRawRequestBody)) shouldBe
    ...
    "return ValueFormatError error (single failure)" when {
      "one field fails value validation (foreign dividend)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidForeignDividendRawRequestBody)) shouldBe
          List(
      ...
      "one field fails value validation (dividend income received whilst abroad)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidDividendIncomeReceivedWhilstAbroadRawRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
      ...
      "one field fails value validation (stock dividend)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidStockDividendRawRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
      ...
      "one field fails value validation (redeemable shares)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidRedeemableSharesRawRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
      ...
      "one field fails value validation (bonus issues of securities)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidBonusIssuesOfSecuritiesRawRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
      ...
      "one field fails value validation (close company loans written off)" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, invalidCloseCompanyLoansWrittenOffRawRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
    ...
    "return ValueFormatError error (multiple failures)" when {
      "multiple fields fail value validation" in new Test {
        validator.validate(CreateAmendDividendsRawData(validNino, validTaxYear, allInvalidValueRawRequestBody)) shouldBe
          List(
    ...
    "return multiple errors" when {
      "request supplied has multiple errors (path parameters)" in new Test {
        validator.validate(CreateAmendDividendsRawData("A12344A", "20178", emptyRawRequestBody)) shouldBe
