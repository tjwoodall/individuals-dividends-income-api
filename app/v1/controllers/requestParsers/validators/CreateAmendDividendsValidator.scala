...
import v1.controllers.requestParsers.validators.validations.CustomerRefInsuranceValidation
import v1.models.request.createAmendDividends._

import javax.inject.{Inject, Singleton}

@Singleton
class CreateAmendDividendsValidator @Inject() (implicit appConfig: AppConfig)
    extends Validator[CreateAmendDividendsRawData]
    with ValueFormatErrorMessages {

  private val validationSet = List(parameterFormatValidation, parameterRuleValidation, bodyFormatValidator, bodyValueValidator)

  override def validate(data: CreateAmendDividendsRawData): List[MtdError] = {
    run(validationSet, data).distinct
  }

  private def parameterFormatValidation: CreateAmendDividendsRawData => List[List[MtdError]] = (data: CreateAmendDividendsRawData) => {
    List(
  ...
  private def parameterRuleValidation: CreateAmendDividendsRawData => List[List[MtdError]] = { data =>
    List(
      TaxYearNotSupportedValidation.validate(data.taxYear, appConfig.minimumPermittedTaxYear)
    )
  }

  private def bodyFormatValidator: CreateAmendDividendsRawData => List[List[MtdError]] = { data =>
    List(
      JsonFormatValidation.validate[CreateAmendDividendsRequestBody](data.body.json)
    )
  }

  private def bodyValueValidator: CreateAmendDividendsRawData => List[List[MtdError]] = { data =>
    val requestBodyData = data.body.json.as[CreateAmendDividendsRequestBody]
  ...
  private def validateForeignDividend(foreignDividend: CreateAmendForeignDividendItem, arrayIndex: Int): List[MtdError] = {
    List(
  ...
  private def validateCommonDividends(commonDividends: CreateAmendCommonDividends, fieldName: String): List[MtdError] = {
    List(
