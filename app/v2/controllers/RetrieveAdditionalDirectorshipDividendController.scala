...
class RetrieveAdditionalDirectorshipDividendController @Inject()(val authService: EnrolmentsAuthService,
  ...
  implicit val endpointLogContext: EndpointLogContext =
    EndpointLogContext(
      controllerName = "RetrieveAdditionalDirectorshipDividendController",
      endpointName = "retrieveAdditionalDirectorshipAndDividendInformation"
    )

  def retrieve(nino: String, taxYear: String, employmentId: String): Action[AnyContent] =
    authorisedAction(nino).async { implicit request =>
      implicit val ctx: RequestContext = RequestContext.from(idGenerator, endpointLogContext)

      val validator = validatorFactory.validator(nino, taxYear, employmentId)
